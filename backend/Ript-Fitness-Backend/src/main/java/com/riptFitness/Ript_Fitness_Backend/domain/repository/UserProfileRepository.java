...
public interface UserProfileRepository extends JpaRepository<UserProfile, Long> {
	...
	// Updates the restDays for a user profile (Maximum number of rest days per
	// week)
	@Modifying
	@Transactional
	@Query("UPDATE UserProfile u SET u.restDays = :restDays WHERE u.username = :username")
	void updateRestDaysByUsername(@Param("username") String username, @Param("restDays") Integer restDays);

	// Retrieves the restDays for a user profile (Maximum number of rest days per
	// week)
	...
	// Updates the restDaysLeft for a user profile (Remaining rest days for the
	// current week)
	@Modifying
	@Transactional
	@Query("UPDATE UserProfile u SET u.restDaysLeft = :restDaysLeft WHERE u.username = :username")
	void updateRestDaysLeftByUsername(@Param("username") String username, @Param("restDaysLeft") Integer restDaysLeft);

	// Retrieves the restDaysLeft for a user profile (Remaining rest days for the
	// current week)
	...
	void updateRestResetDateByUsername(@Param("username") String username,
			@Param("restResetDate") LocalDate restResetDate);
	...
	// Updates the restResetDayOfWeek for a user profile (Day of the week when rest
	// resets)
	@Modifying
	@Transactional
	@Query("UPDATE UserProfile u SET u.restResetDayOfWeek = :restResetDayOfWeek WHERE u.username = :username")
	void updateRestResetDayOfWeekByUsername(@Param("username") String username,
			@Param("restResetDayOfWeek") Integer restResetDayOfWeek);

	// Retrieves the restResetDayOfWeek for a user profile (Day of the week when
	// rest resets)
	...
	List<UserProfile> findByUsernameContainingIgnoreCaseAndNotUsername(@Param("searchTerm") String searchTerm,
			@Param("currentUsername") String currentUsername, Pageable pageable);
