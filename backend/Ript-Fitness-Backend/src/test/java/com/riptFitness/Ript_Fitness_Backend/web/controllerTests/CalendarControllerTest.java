...
public class CalendarControllerTest {
	...
	public void testLogWorkoutDay() throws Exception {
		mockMvc.perform(post("/calendar/logWorkout").header("Authorization", "Bearer test-token")
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
	}

	@Test
	public void testLogRestDay() throws Exception {
		mockMvc.perform(post("/calendar/logRestDay").header("Authorization", "Bearer test-token")
				.contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
	}
